...
**Answer** There are a few NAs for certain years and this means that they changed conferences. Especially now with conference realignment going nuts it will be difficult to track periodocially. 
...
**Answer** Hawaii had the msot growth at 366%. There are also three MWC teams in the top 10. Some teams who are traditionally not good at football were up there. 
...
matches_2023 |> 
  group_by(team) |> 
  summarise( total_on_goal = sum(so_g), total_goals = sum(goals), mean_goals = mean(goals)) |> 
  mutate(percent = ((total_on_goal - total_goals)/total_on_goal)*100) |>
  filter(total_goals > 2) %>% 
  arrange(desc(percent))
```

What stands out to you here? Describe that below.
Would you narrow down the results to certain teams based on the data? 
If so, do that in the code and explain why you did it below. 
Finally, what's the most interesting result to you that could be a story?

**Answer** I wanted to find the average number of goals so I added a mean_goals column to the tibble. Many of the teams didnt have any goals and I filtered them out to get a more accurate percentage. I filtered out any team that had two or fewer goals. Some teams on the list are notable but not increadibly many. 

Also, Im a bit confused about how to interpret the percentage. For the first team, would you say the percent change between shots on goal and total goals is 91%?

I dont know exactly how the date would be relevant. You could, in theory, narrow the results based on the month to see how many goals or shots on goal a team has in a given month or week.

An interesting story here could be how many teams with "x" goals or fewer have a certain amount of wins? I feel like that may be too broad of a question but could get at the idea?
