...
select(Institution, Conference, `2022`)
```
...
players |>
  ...
  filter(PER > 12, GS > 10) %>% 
  arrange(desc(trueshooting)) %>% 
  select(Team, Player, PER, GS, trueshooting)
```

**ANSWER** Who do you get as the first result, and what was his trueshooting percentage?

My answer is Matthew Marsh of Wake Forest with a trueshooting percengage of 81% and an efficiency rating of 19.3. The mean PER was 12 so I took a list of players above that mark and also stated 10 or more games. 
