...
```
**Answer** Produces a dataframe with 679 rows and 27 columns. The dataset has NBA players and some of them are in multiple rows. I believe this is because they switched teams during the season adn TOT means their total stats for the seaosn. 
...
**Answer** This group_by function produces a 2x2 tibble showing how many players are in each position. The tibble is not in descending order.
...
**Answer** The complicating result is that there are players who play multiple positions and there are different combinations of these. In theory, any player can play any position so I would think we could mutate the data to show players who use a single position.
...
**Small Forwards played the most minutes along with Power Forwards. 952 players are listed at both positions. I'd like to see which players are better at each position. Although, better is subjective. Carmelo Anthony is someone who played power forawrd and also small forward. So does LeBron. There could be an argument made that these two players help their teams more when they play one position vs. another position. ** 