...
winner_loser <- game_boxes_with_teams %>% 
  select(home_team_name, home_team_conference, pts, away_team_name, away_team_conference, opp_pts, winner, winning_conference, difference)
```
...
ator_only <- game_boxes_with_teams %>% 
    select(gamedate_east, home_team_name, home_team_conference, pts, away_team_name, away_team_conference, opp_pts, winner, winning_conference, ast, tov, opp_ast, opp_tov, team_ator, opp_ator, difference)
```
...
b10_only <- ggplot() +
  ...
  geom_smooth(data=total_wins, aes(x=total_wins, y=avg_ator), method="lm") +
...
ggplot() + 
  geom_point(
    data=b10, 
    aes(x=avg_ator, y=avg_opp_ator, size = total_wins))+
  geom_text_repel(
    data=b10, 
    aes(x=avg_ator, y=avg_opp_ator, label=winner))+
  labs(title="Big Ten at large proves ator leads to wins",
       subtitle="Many Big Ten opponents also play efficient basketball",
       caption="Source: Her Hoops Stats | Graph by Dylan Manfre",
       x = "Average Team ator",
       y = "Average Opponent ator")
```
...
```{r}
model1 <- lm(total_wins ~ avg_ator, data = total_wins)
summary(model1)
```
...
umd <- ator_only %>% 
  ...
  select(gamedate_east, home_team_name, away_team_name, winner, team_ator, opp_ator) %>% 
...
Its best ator was a game against UCF when it had an ator of a 4.0 which is huge. They had 24 assits that game. UCF only had 11 assists and 15 turnovers. That particular game displayed what Tennessee is able to do on defense and how it can capitalize off opponents mistakes. 
...
Looking at how it's spread out over time, the Vols stayed pretty consistent averaging over 1 for most of the season November was a 1.0 but it went 4-8 that month. December was it's best month averaging a 1.7 ator.
...
ggplot() + geom_line(data=tenn, aes(x=gamedate_east, y=team_ator))+
            geom_hline(data=p5_conf, aes(yintercept=1.070761 ,color="red"))+
  labs(title="Tennessee is taking care of the ball",
       ...
       x = "Month",
       y = "Team ator",
       color = "SEC Average ATOR")
```

```{r}
#Tenn player scrapping
...
The Big 12 is right in the center with a respectable ator, but the fewest wins.

```{r}
p5_plot <- ggplot() + 
  geom_point(
    data=p5_conf, 
    aes(x=total_wins, y=avg_ator))+
  geom_text_repel(
    data=p5_conf, 
    aes(x=total_wins, y=avg_ator, label=winning_conference))+
  labs(title="Does a good ATOR really lead to more wins?",
       subtitle="The short answer is, it depends, Big 12 shows otherwise.",
       caption="Source: Her Hoops Stats | Graph by Dylan Manfre",
       x = "Wins",
       y = "Average Team ator")
```

There's a few characters at play here that both prove Frese's point and also dispel it. Clearly, having a good assist to turnover ratio does lead to wins in most cases. Every coach has their one aspect that they like to harp on. ator is hers and for Maryland, it's worked. The veteran head caoch hopes it'll work again this year as the Terps hope to make it back to the Elite Eight with a fresh crop of players.
