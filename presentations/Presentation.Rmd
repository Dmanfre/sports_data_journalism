...
total_wins <- game_boxes_with_teams %>%
  ...
  summarize(
    ...
    avg_losing_ator = mean(losing_ator)
  ) %>%
  arrange(desc(total_wins))

#This shows me the number of wins a team has, its avg ator in wins and a team's avg ator in losses. 
```
...
time_element <- ator_only %>% 
  ...
  group_by(month) %>%
  summarize(avg_ator = mean(winning_ator),
...
```{r}
b10_only <- ggplot() +
  geom_point(data = b10, aes(x = total_wins, y = avg_winning_ator)) +
  geom_smooth(data=total_wins, aes(x=total_wins, y=avg_winning_ator), method="lm") +
...
p5_conf <- p5_teams_ator %>%
  ...
  arrange(desc(avg_winning_ator))

ggplot() + 
  geom_point(
    data=b10, 
    aes(x=avg_winning_ator, y=avg_losing_ator, size = total_wins))+
  geom_text_repel(
    data=b10, 
    aes(x=avg_winning_ator, y=avg_losing_ator, label=winner))+
...
model1 <- lm(total_wins ~ avg_winning_ator, data = total_wins)
summary(model1)
```
...
umd <- ator_only %>% 
  ...
  select(gamedate_east, home_team_name, pts, away_team_name, opp_pts, team_ator, opp_ator, winner, winning_ator, losing_ator)
```
...
b10vSEC <- ggplot() +
  geom_point(data = b10, aes(x = total_wins, y = avg_winning_ator), color = "red") +
  geom_point(data = sec, aes(x = total_wins, y = avg_winning_ator), color = "blue") +
  geom_smooth(data=total_wins, aes(x=total_wins, y=avg_winning_ator), method="lm") +
  ...
  geom_text_repel(data = b10, aes(x = total_wins, y = avg_winning_ator, label = winner))+
  geom_text_repel(data = sec, aes(x = total_wins, y = avg_winning_ator, label = winner))
...
Looking at how it's spread out over time, the Vols stayed pretty consistent averaging over 1 for most of the season November was a 1.21 but it went 4-8 that month. December was it's best month averaging a 1.9 ator.

```{r}
tenn_timing <- tenn %>% 
  filter(winner == "Tennessee") %>% 
  mutate(month = month(gamedate_east)) %>% 
  group_by(month) %>% 
  summarize(avg_winning_ator = mean(winning_ator))
```

```{r}
ggplot() + geom_line(data=tenn, aes(x=gamedate_east, y=winning_ator))+
...
```{r}
tn_away <- ator_only %>% 
  filter(away_team_name == "Tennessee")

tn_away_ast <- sum(tn_away$opp_ast)
tn_away_turn <- sum(tn_away$opp_tov)
...
tn_home <- ator_only %>% 
  filter(home_team_name == "Tennessee")

tn_home_ast <- sum(tn_home$ast)
tn_home_turn <- sum(tn_home$tov)

tenn_ator = (tn_away_ast + tn_home_ast)/(tn_away_turn + tn_home_turn)
