...
new_col_names <- c("Country", "F_23_24", "C_23_24", "G_23_24", "G_F_23_24", "F_22_23", "C_22_23", "G_22_23", "G_F_22_23")

#ChatGPT showed this backwards and I was curious why
colnames(total_position_by_country) <- new_col_names

#Columns 11-13 were blank and duplicates
...
center_subset <- total_position_by_country %>% 
  select(Country, C_23_24, C_22_23, C_diff) %>% 
  head(25)
...
Create this based on charts of the countries dataframe and the the positions but limit it to a scope of the most interesting finds. Do dumbell charts

```{r}
#Position subset

position_subset <- total_position_by_country %>% 
  select(C_23_24, C_22_23, C_diff) %>% 
  arrange(desc(C_diff)) %>% 
  head(10)
...
ggplot() + 
  geom_lollipop(
    data=center_subset, 
    aes(y=reorder(Country, C_diff), x=C_diff), 
    horizontal = TRUE
    )
