...
# first try I did this, the team_ids did not go over. 
...
two options
- i can only take ones that match
- can find out all unique teams that didnt match. create look up table, 
```{r}
...
na_teams %>%
  mutate(ncaa_id = case_when(
    team.x == "La Salle" ~ "340",
    team.x == "Loyola (IL)" ~ "371",
    team.x == "Saint Joseph's" ~ "606",
    team.x == "Jacksonville State" ~ "315",
    team.x == "UTSA" ~ "706",
    team.x == "Miami (FL)" ~ "415",
    team.x == "Albany (NY)" ~ "14",
    team.x == "Maryland-Baltimore County" ~ "391",
    team.x == "Massachusetts-Lowell" ~ "368",
    team.x == "NJIT" ~ "471",
    team.x == "TCU" ~ "698",
    team.x == "Connecticut" ~ "164",
    team.x == "St. John's (NY)" ~ "603",
    team.x == "Idaho State" ~ "294",
    team.x == "Presbyterian" ~ "1320",
    team.x == "South Carolina Upstate" ~ "10411",
    team.x == "Cal State Northridge" ~ "101",
    team.x == "North Carolina A&T" ~ "488",
    team.x == "UNC Wilmington" ~ "460",
    team.x == "Florida International" ~ "231",
    team.x == "Jacksonville State" ~ "315",
    team.x == "Sam Houston" ~ "624",
    team.x == "UTEP" ~ "704",
    team.x == "UTSA" ~ "706",
    team.x == "Western Kentucky" ~ "772",
    team.x == "Purdue Fort Wayne" ~ "308",
    team.x == "Wright State" ~ "810",
    team.x == "Youngstown State" ~ "817",
    team.x == "Bowling Green State" ~ "71",
    team.x == "Miami (OH)" ~ "414",
    team.x == "Morgan State" ~ "446",
    team.x == "South Carolina State" ~ "647",
    team.x == "Missouri State" ~ "669",
    team.x == "Southern Illinois" ~ "ID21",
    team.x == "Colorado State" ~ "ID22",
    team.x == "Nevada-Las Vegas" ~ "ID23",
    team.x == "Utah State" ~ "ID24",
    team.x == "Central Connecticut State" ~ "ID25",
    team.x == "Le Moyne" ~ "ID26",
    team.x == "Long Island University" ~ "ID27",
    team.x == "St. Francis (NY)" ~ "ID28",
    team.x == "Lindenwood" ~ "ID29",
    team.x == "Little Rock" ~ "ID30",
    team.x == "SIU Edwardsville" ~ "ID31",
    team.x == "Southern Indiana" ~ "ID32",
    team.x == "Tennessee State" ~ "ID33",
    team.x == "Tennessee-Martin" ~ "ID34",
    team.x == "Southern California" ~ "ID35",
    team.x == "Boston University" ~ "ID36",
    team.x == "Loyola (MD)" ~ "ID37",
    team.x == "Louisiana State" ~ "ID38",
    team.x == "Jackson State" ~ "ID39",
    team.x == "Prairie View" ~ "ID40",
    team.x == "Texas A&M-Commerce" ~ "ID41",
    team.x == "Texas A&M-Corpus Christi" ~ "ID42",
    team.x == "St. Thomas" ~ "ID43",
    team.x == "Arkansas State" ~ "ID44",
    team.x == "Louisiana-Monroe" ~ "ID45",
    team.x == "Southern Mississippi" ~ "ID46",
    team.x == "Tarleton State" ~ "ID47",
    team.x == "Texas-Rio Grande Valley" ~ "ID48",
    team.x == "UT Arlington" ~ "ID49",
